...
def get_search_space():
    search_space = [
        skopt.space.Integer(1, 10, "uniform", name="embed_pow"),  # window size
        skopt.space.Integer(
        ...
        # skopt.space.Integer(
        #     1, 5, "uniform", name="max_len"
        # ),  # negative sampling items selection
        skopt.space.Integer(4, 64, "log-uniform", name="vocab_size"),  # learning rate
        skopt.space.Real(
            1e-8, 1.0, "log-uniform", name="lr"
        ),  # high-freq words temperature
        skopt.space.Real(
            0.0, 1.0, "uniform", name="entropy_coeff"
        ),  # smallest value to learning rate
